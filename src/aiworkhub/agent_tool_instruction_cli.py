...
Usage:
  python -m aiworkhub.agent_tool_instruction_cli preview [--repo ROOT] [--provider P]
  python -m aiworkhub.agent_tool_instruction_cli inspect [--repo ROOT] --provider P
  python -m aiworkhub.agent_tool_instruction_cli apply  [--repo ROOT] --provider P [--write]

Default is dry-run / preview.  ``apply --write`` requires
``AIWORKHUB_ALLOW_WRITES=1`` in the environment.
"""
