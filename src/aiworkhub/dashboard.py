"""Canonical builders and loopback HTTP server for the AIWorkHub dashboard.

The native VS Code Webview (see ``vscode-extension/extension.js``) talks to
this package over the repository-local Task MCP stdio session -- there is no
HTTP listener requirement for the native path. The loopback server remains a
supported local operator surface and uses the same repository-bound builders.
Its only mutations are capability-gated archive/restore operations against the
bound canonical task store.
"""
...
def parse_loopback_authority(value: str) -> tuple[str, int | None] | None:
    """Parse a Host-style authority without attacker-controlled DNS."""
...
def parse_dashboard_authority(value: str) -> tuple[str, int | None] | None:
    ...
    allowed = {
        item.strip()
        for item in os.environ.get("AIWORKHUB_DASHBOARD_ALLOWED_HOSTS", "").split(",")
        if item.strip()
    }
...
def _provider_repo_root(provider: Any) -> Path:
    root = getattr(provider, "repo_root", None)
    return Path(root).resolve() if root is not None else _default_repo_root()


class DashboardRequestHandler(BaseHTTPRequestHandler):
    server_version = "AIWorkHubDashboard/1.0"
    sys_version = ""
    ...
    def _serve_static(self, path: str, *, send_body: bool, vscode_embed: bool = False) -> None:
        ...
        if resolved is None:
            self._send_json(
                HTTPStatus.NOT_FOUND, {"ok": False, "error": "not_found"}, send_body=send_body
            )
            return
    ...
    def _send_identity(self, *, send_body: bool) -> None:
        try:
            identity = task_store.database_identity(
                _provider_repo_root(self.server.provider)  # type: ignore[attr-defined]
            )
    ...
    def _dispatch(self, *, send_body: bool) -> None:
        ...
        if path == "/api/health-identity":
            if parsed.query:
                self._bad_request(
                    "health identity does not accept query parameters", send_body=send_body
                )
                return
            self._send_identity(send_body=send_body)
            return
        if path == "/api/task":
            try:
            ...
            try:
                detail = build_task_detail(
                    task_id, self.server.provider  # type: ignore[attr-defined]
                )
            except Exception as exc:
                self._send_json(
                    HTTPStatus.SERVICE_UNAVAILABLE,
        ...
        if path == VSCODE_EMBED_PATH:
            if parsed.query:
                self._bad_request(
                    "vscode embed endpoint does not accept query parameters", send_body=send_body
                )
                return
    ...
    def _verify_coordinator_token(self) -> str:
        token_header = (self.headers.get("X-Coordinator-Token") or "").strip()
        if not token_header:
            return "missing X-Coordinator-Token header"
        try:
            from . import coordinator_config
        ...
        token_path = (
            ...
            if configured_path
            else Path.home() / ".config" / "aiworkhub" / "taskctl_coordinator.token"
        ).resolve()
        try:
        ...
        if not hmac.compare_digest(token_header, valid_token):
            return "invalid coordinator token"
        return ""

    def _handle_archive(self, task_id: str, reason: str) -> tuple[int, dict[str, Any]]:
        try:
            ok, message = task_store.archive_task(
                _provider_repo_root(self.server.provider),  # type: ignore[attr-defined]
                task_id,
                actor="dashboard",
                reason=reason,
        ...
        if ok:
            return HTTPStatus.OK, {"ok": True, "task_id": task_id, "message": message}
        status = HTTPStatus.NOT_FOUND if message == "task_not_found" else HTTPStatus.CONFLICT
        return status, {"ok": False, "error": message, "task_id": task_id}

    def _handle_restore(self, task_id: str, reason: str) -> tuple[int, dict[str, Any]]:
        try:
            ok, message = task_store.restore_task(
                _provider_repo_root(self.server.provider),  # type: ignore[attr-defined]
                task_id,
                actor="dashboard",
                reason=reason,
    ...
    def _dispatch_post(self, *, send_body: bool) -> None:
        ...
        try:
            ...
            self._bad_request("invalid request target", send_body=send_body)
            return
        if parsed.path in ("/api/archive", "/api/restore"):
            ...
            if parsed.path == "/api/archive":
                status, response = self._handle_archive(task_id, reason)
            else:
                status, response = self._handle_restore(task_id, reason)
            self._send_json(status, response, send_body=send_body)
            return
        if parsed.path == "/api/health-identity":
            self._send_identity(send_body=send_body)
            return
        self.close_connection = True
        self._send_json(
            HTTPStatus.METHOD_NOT_ALLOWED,
            {"ok": False, "error": "method_not_allowed", "message": "dashboard is read-only"},
            send_body=send_body,
    ...
    def _reject_mutation(self) -> None:
        ...
        self._send_json(
            HTTPStatus.METHOD_NOT_ALLOWED,
            {"ok": False, "error": "method_not_allowed", "message": message},
            send_body=True,
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Serve the local AIWorkHub task dashboard")
    ...
    print(f"AIWorkHub task dashboard: {url}", flush=True)
    if not args.no_open:
