...
Hard invariants (asserted by test_launch_queue_persist_audit_b119_v1.sh):
  ...
  * Isolation-safe / parallel-safe: the log path is resolvable from an explicit
    argument, then ``GEOAI_TASK_MCP_LAUNCH_QUEUE_LOG_PATH``, then a repo-root
...
_CONTRACT_MODULE_NAME = "geoai_task_mcp._launch_queue_contract_b119_persist"
...
# ---------------------------------------------------------------------------
LAUNCH_QUEUE_LOG_ENV: str = "GEOAI_TASK_MCP_LAUNCH_QUEUE_LOG_PATH"
...
def default_repo_root() -> Path:
    return Path(os.environ.get("GEOAI_REPO", "/home/shrek/GeoAI")).expanduser().resolve()
