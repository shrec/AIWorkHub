...
coordinator's ``server.py`` (GeoAI Task MCP). It is what the parent launcher
...
``.aiworkinghub/config/storage.json`` registry (read from ``ctx.authority_repo``,
...
SERVER_NAME = "geoai_worker_ai_tools"

AUDIT_ENTRY_SCHEMA_ID = "geoai.task_mcp.worker_mcp_audit_entry.v1"
AUDIT_LEDGER_VERIFICATION_SCHEMA_ID = "geoai.task_mcp.worker_mcp_audit_verification.v1"
RUNTIME_SCHEMA_ID = "geoai.task_mcp.worker_mcp_runtime.v1"

ENV_TASK_ID = "GEOAI_WORKER_MCP_TASK_ID"
ENV_RUNNER = "GEOAI_WORKER_MCP_RUNNER"
ENV_TOPIC = "GEOAI_WORKER_MCP_TOPIC"
ENV_REQUEST_ID = "GEOAI_WORKER_MCP_REQUEST_ID"
ENV_REPO = "GEOAI_WORKER_MCP_REPO"
ENV_AUTHORITY_REPO = "GEOAI_WORKER_MCP_AUTHORITY_REPO"
ENV_SOURCE_GRAPH_TARGETS = "GEOAI_WORKER_MCP_SOURCE_GRAPH_TARGETS"
ENV_SESSION_TOPIC = "GEOAI_WORKER_MCP_SESSION_TOPIC"
ENV_AUDIT_LEDGER_PATH = "GEOAI_WORKER_MCP_AUDIT_LEDGER_PATH"
ENV_AUDIT_HMAC_KEY_PATH = "GEOAI_WORKER_MCP_AUDIT_HMAC_KEY_PATH"
...
STORAGE_REGISTRY_RELATIVE_PATH = Path(".aiworkinghub") / "config" / "storage.json"
...
def _canonical_json_output(name: str, text: str, *, max_bytes: int) -> tuple[str, bool]:
    ...
    wrapper = {
        "schema_id": "geoai.task_mcp.bounded_json_preview.v1",
        "truncated": True,
...
def _resolve_authority_db(ctx: WorkerToolContext, *, component: str, db_id: str) -> AuthorityBinding:
    ...
    if authority.get("canonical_active"):
        ...
        durable_root = str(registry.get("durable_root") or ".aiworkinghub")
...
def source_graph_query(
    ...
    *,
    ...
    try:
        binding = _resolve_authority_db(ctx, component="source_graph", db_id="source_graph")
    except WorkerToolError as exc:
        return _violation(ctx, tool, str(exc)[:160])

    script = ctx.authority_repo / "AITools" / "source_graph.py"
    if not script.is_file():
        return _violation(ctx, tool, "source_graph_tool_missing")
    if mode == "bundle":
        argv = [
            sys.executable, str(script), "bundle", bundle_type, bounded_query,
            "--max-lines", str(budget), "--json",
        ]
    else:
        argv = [sys.executable, str(script), mode, bounded_query, str(budget), "--json"]
...
def register_tools(mcp: Any, ctx: WorkerToolContext) -> tuple[str, ...]:
    ...
    @mcp.tool(name="geoai_worker_source_graph_query")
    ...
    @mcp.tool(name="geoai_worker_session_current_state")
...
def build_server(ctx: WorkerToolContext) -> Any:
    ...
    mcp = FastMCP("GeoAI Worker AI Tools")
