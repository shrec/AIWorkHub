...
CREDENTIAL_PATH_ENV = "GEOAI_TASK_MCP_DEEPSEEK_CREDENTIAL"
_DEFAULT_CREDENTIAL_REL = Path(".config") / "geoai-task-mcp" / "deepseek_copilot_credential.json"
...
def credential_path(path: os.PathLike[str] | str | None = None) -> Path:
    ...
    Priority: explicit ``path`` argument > ``GEOAI_TASK_MCP_DEEPSEEK_CREDENTIAL``
    env override > ``~/.config/geoai-task-mcp/deepseek_copilot_credential.json``.
    """
    if path is not None:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="geoai-task-deepseek-credential",
