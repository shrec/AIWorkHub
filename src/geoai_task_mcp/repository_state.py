"""Repository-local AIWorkingHub durable state foundation.

This module is intentionally import-safe: it performs no filesystem reads or
writes at import time and does not depend on the legacy GeoAI checkout path.
...
HUB_DIRNAME = ".aiworkinghub"
PROJECT_MANIFEST_REL = Path(HUB_DIRNAME) / "project.json"
MANIFEST_SCHEMA_ID = "aiworkinghub.project_manifest.v1"
MANIFEST_VERSION = 1
LAYOUT_VERSION = 1
...
class ManifestMissingError(RepositoryStateError):
    """A repository root exists, but no AIWorkingHub manifest is present."""
...
def resolve_repository_root(
    ...
    *,
    ...
    Precedence is: explicit_root, AIWORKINGHUB_REPO_ROOT env, cwd ancestor with
    .aiworkinghub/project.json, then cwd git root.  When require_manifest is
    ...
    elif environ.get("AIWORKINGHUB_REPO_ROOT"):
        root = _resolve_existing_dir(Path(str(environ["AIWORKINGHUB_REPO_ROOT"])))
    else:
...
def discover_legacy_candidates(root: str | Path) -> tuple[dict[str, str], ...]:
    """Return legacy GeoAI locations as read-only migration candidates only."""
...
def bootstrap_repository(
    ...
    *,
    ...
    """Create .aiworkinghub/project.json and canonical directories.
