...
Hard invariants (asserted by launch_disabled_queue_contract_smoke.py):
  ...
  * ``launch_enabled()`` is always ``False``. A future launch-capable tool must
    require BOTH ``GEOAI_TASK_MCP_ALLOW_LAUNCH=1`` AND
    ``GEOAI_TASK_MCP_ALLOW_WRITES=1`` (both default '0' = disabled). This module
...
Neural-bridge note (GeoAI neural-control-first doctrine): the FUTURE selection of
...
# ---------------------------------------------------------------------------
ALLOW_LAUNCH_ENV: str = "GEOAI_TASK_MCP_ALLOW_LAUNCH"
ALLOW_WRITES_ENV: str = "GEOAI_TASK_MCP_ALLOW_WRITES"
...
def launch_flag_set() -> bool:
    """True only when GEOAI_TASK_MCP_ALLOW_LAUNCH == '1' (default off)."""
    return os.environ.get(ALLOW_LAUNCH_ENV, DISABLED_DEFAULT) == ENABLED_VALUE


def writes_allowed() -> bool:
    """True only when GEOAI_TASK_MCP_ALLOW_WRITES == '1' (default off).
