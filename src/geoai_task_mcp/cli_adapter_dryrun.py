"""DRYRUN-ONLY local CLI adapter contract for the GeoAI task MCP.
...
Hard invariants (asserted by tests):
  ...
  * The write gate is OFF by default (``GEOAI_TASK_MCP_ALLOW_WRITES != '1'``);
...
ADAPTER_ARGV_TEMPLATES: dict[str, list[str]] = {
    "claude_cli": ["claude", "-p", "<prompt>", "--cwd", "<GEOAI_REPO>"],
    "codex_cli": ["codex", "exec", "<prompt>", "--cwd", "<GEOAI_REPO>"],
...
def build_argv_template(adapter_id: str, prompt: str, repo: str | None = None) -> list[str]:
    ...
    Substitutes ``<prompt>`` and ``<GEOAI_REPO>`` placeholders. Returns [] for
