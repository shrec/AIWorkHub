...
CREDENTIAL_PATH_ENV = "GEOAI_TASK_MCP_GLM_CREDENTIAL"
_DEFAULT_CREDENTIAL_REL = Path(".config") / "aiworkinghub" / "glm_copilot_credential.json"
...
def credential_path(path: os.PathLike[str] | str | None = None) -> Path:
    ...
    Priority: explicit ``path`` argument > ``GEOAI_TASK_MCP_GLM_CREDENTIAL`` >
    ``~/.config/aiworkinghub/glm_copilot_credential.json``.
    """
    if path is not None:
...
def credential_status(
    *,
    ...
    return {
        ...
        "credential_path_default": "~/.config/aiworkinghub/glm_copilot_credential.json",
        "setup_command": "python3 -m geoai_task_mcp.glm_credentials setup",
        "status_command": "python3 -m geoai_task_mcp.glm_credentials status",
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog="geoai-task-glm-credential",
