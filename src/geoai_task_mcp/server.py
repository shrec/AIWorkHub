...
mcp = FastMCP("GeoAI Task MCP")


@mcp.tool()
def geoai_task_health() -> dict[str, Any]:
    """Check parent GeoAI task queue health and write-gate state."""
...
@mcp.tool()
def geoai_task_review_queue() -> dict[str, Any]:
...
@mcp.tool()
def geoai_task_list(status: str = "pending", topic: str | None = None, limit: int = 80) -> dict[str, Any]:
...
@mcp.tool()
def geoai_task_show(task_id: str) -> dict[str, Any]:
...
@mcp.tool()
def geoai_task_pending_for_runner(runner: str, topic: str | None = None) -> dict[str, Any]:
...
@mcp.tool()
def geoai_task_auto_pickup(runner: str, topic: str | None = None) -> dict[str, Any]:
...
@mcp.tool()
def geoai_task_auto_pickup_dryrun(runner: str, topic: str | None = None) -> dict[str, Any]:
    ...
    `export` path. The real claim path (geoai_task_auto_pickup) stays behind
    GEOAI_TASK_MCP_ALLOW_WRITES; this preview provides no alternate write path
...
@mcp.tool()
def geoai_task_mark_review(task_id: str) -> dict[str, Any]:
...
@mcp.tool()
def geoai_task_mark_done(task_id: str) -> dict[str, Any]:
...
@mcp.tool()
def geoai_task_collision_guard(print_json: bool = True) -> dict[str, Any]:
...
@mcp.tool()
def geoai_task_export_jsonl() -> dict[str, Any]:
...
@mcp.tool()
def geoai_task_audit_log_read(max_entries: int = 100) -> dict[str, Any]:
...
def geoai_task_codex_handoff(
    ...
    result["server_tool"] = "geoai_task_codex_handoff"
    return result


@mcp.tool()
def geoai_task_codex_handoff_markdown(
    task_ids: list[str] | None = None,
    batch_label: str | None = None,
) -> dict[str, Any]:
    """READ-ONLY: Codex-ready HANDOFF report rendered as compact Markdown.

    Builds the same B116 handoff report as ``geoai_task_codex_handoff`` (via
    ...
    result["server_tool"] = "geoai_task_codex_handoff_markdown"
    return result


@mcp.tool()
def geoai_supervisor_loop_status(
    task_id: str,
    runner: str | None = None,
    topic: str | None = None,
...
# ---------------------------------------------------------------------------
# B821: AIWorkingHub agent tool-instruction activation (read-only-first MCP tools).
...
# parent write gate (GEOAI_TASK_MCP_ALLOW_WRITES=1).
# ---------------------------------------------------------------------------
...
#     writes nothing regardless of GEOAI_TASK_MCP_ALLOW_WRITES.
# ---------------------------------------------------------------------------
...
@mcp.tool()
def geoai_launch_queue_describe_readonly(
    task_id: str,
    runner: str,
    topic: str,
    adapter_id: str,
    ...
    result["server_tool"] = "geoai_launch_queue_describe_readonly"
    return result


@mcp.tool()
def geoai_launch_queue_evaluate_readonly(
    task_id: str,
    runner: str,
    topic: str,
    adapter_id: str,
    ...
    decision["server_tool"] = "geoai_launch_queue_evaluate_readonly"
    return decision


@mcp.tool()
def geoai_launch_queue_audit_summary_readonly(max_entries: int = 100) -> dict[str, Any]:
    ...
    never launches a process. Same shape as ``geoai_task_audit_log_read``:
    ...
    state while ``GEOAI_TASK_MCP_ALLOW_LAUNCH`` / ``_ALLOW_WRITES`` stay unset
    (the default).
    ...
    result["server_tool"] = "geoai_launch_queue_audit_summary_readonly"
    return result


@mcp.tool()
def geoai_task_queue_request(
    task_id: str,
    runner: str,
    topic: str,
    adapter_id: str,
    ...
    ``GEOAI_TASK_MCP_ALLOW_WRITES`` unset, it appends nothing. With the write
...
@mcp.tool()
def geoai_task_stale_recovery_recommend(
    topic: str | None = None,
    limit: int = 200,
) -> dict[str, Any]:
...
def geoai_agent_launch_task(
    ...
    Requires both GEOAI_TASK_MCP_ALLOW_LAUNCH=1 and
    GEOAI_TASK_MCP_ALLOW_WRITES=1. The task card, runner, topic, pending state,
...
@mcp.tool()
def geoai_agent_task_status(request_id: str) -> dict[str, Any]:
...
@mcp.tool()
def geoai_agent_collect_result(
    request_id: str,
...
@mcp.tool()
def geoai_agent_list_processes(limit: int = 100) -> dict[str, Any]:
