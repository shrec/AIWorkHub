...
DEFAULT_REPO = Path(os.environ.get("GEOAI_REPO", "/home/shrek/GeoAI"))
TASKCTL_REL = Path("AITools/taskctl.py")
DEFAULT_TIMEOUT_SECONDS = int(os.environ.get("GEOAI_TASK_MCP_TIMEOUT", "60"))
...
DEFAULT_COORDINATOR_TOKEN_FILE = Path.home() / ".config/geoai/taskctl_coordinator.token"

# The actual env-var pop happens at package-init time (see
# geoai_task_mcp/__init__.py::refresh_coordinator_config) -- before this
...
#
# This layer only ever NARROWS an already-open GEOAI_TASK_MCP_ALLOW_WRITES
...
def repo_root() -> Path:
    return Path(os.environ.get("GEOAI_REPO", str(DEFAULT_REPO))).expanduser().resolve()
...
def writes_allowed() -> bool:
    return os.environ.get("GEOAI_TASK_MCP_ALLOW_WRITES", "0") == "1"


def scrub_coordinator_capability_from_environment() -> None:
    """Move late-bound coordinator configuration out of the inherited env.

    Delegates to the package-level scrub (``geoai_task_mcp.refresh_coordinator_config``)
...
def _audit_log_path(repo: Path | None = None) -> Path:
    ...
    env_path = os.environ.get("GEOAI_TASK_MCP_AUDIT_LOG_PATH", "")
    if env_path:
...
def _sanitize_env_for_audit() -> dict[str, str]:
    ...
    relevant = ["GEOAI_TASK_MCP_ALLOW_WRITES", "GEOAI_TASK_MCP_AUDIT_LOG_PATH"]
...
def write_audit_entry(
    ...
    *,
    ...
    try:
        ...
        print(
            f"[geoai-task-mcp] WARNING: audit log write failed: {exc}",
            file=sys.stderr,
        )
...
def run_taskctl(
    ...
    *,
    ...
    ``GEOAI_TASK_MCP_ALLOW_WRITES`` gate is already open -- this layer
    ...
    if _is_write_command(args):
        if not (allow_write and writes_allowed()):
            blocked_reason = (
                "write command blocked; set GEOAI_TASK_MCP_ALLOW_WRITES=1 "
...
def review_queue() -> dict[str, Any]:
    return run_taskctl(["review-queue"]).as_dict()
...
def list_tasks(status: str = "pending", topic: str | None = None, limit: int = 80) -> dict[str, Any]:
    ...
    return run_taskctl(args).as_dict()


def show_task(task_id: str) -> dict[str, Any]:
    return run_taskctl(["show", task_id]).as_dict()


def pending_for_runner(runner: str, topic: str | None = None) -> dict[str, Any]:
    args = ["export", "--runner", runner]
    if topic:
        args.extend(["--topic", topic])
    result = run_taskctl(args).as_dict()
    result["jsonl_rows"] = _parse_jsonl(result.get("stdout", ""))
    return result


def auto_pickup(runner: str, topic: str | None = None) -> dict[str, Any]:
    args = ["auto-pickup", "--runner", runner]
    if topic:
        args.extend(["--topic", topic])
    return run_taskctl(args, allow_write=True, runner=runner, topic=topic).as_dict()


def claim_start_exact(
    task_id: str, runner: str, topic: str, request_id: str = ""
) -> dict[str, Any]:
    """Dual-gate caller helper for taskctl's exact atomic claim/start path."""
    args = ["claim-start", task_id, "--runner", runner, "--topic", topic]
    if request_id:
...
# behind the existing ``GEOAI_TASK_MCP_ALLOW_WRITES`` gate; this preview offers
...
def auto_pickup_dryrun(runner: str, topic: str | None = None) -> dict[str, Any]:
    ...
    Contract (all invariants asserted by the B118 smoke test):
      ...
      * Does NOT bypass ``GEOAI_TASK_MCP_ALLOW_WRITES``; the real claim path
        (``auto_pickup`` / ``geoai_task_auto_pickup``) stays write-gated.
    """
    ...
    return {
        ...
        "dry_run": True,
        "tool": "geoai_task_auto_pickup_dryrun",
...
def queue_request(
    *,
    ...
    """
    from geoai_task_mcp import completion_inbox, launch_queue_contract, launch_queue_persist
    ...
    base = {
        ...
        "server_tool": "geoai_task_queue_request",
    }
    ...
    if not writes_allowed():
        return {
            **base,
            "ok": False,
            "blocked_reason": "write_gate_closed:GEOAI_TASK_MCP_ALLOW_WRITES!=1",
            "idempotent_noop": False,
            "duplicate_runner_blocked": False,
            "stale_recovery_requeue": False,
...
def mark_done(task_id: str, runner: str | None = None, topic: str | None = None) -> dict[str, Any]:
    ...
    if topic is not None and topic != live_topic:
        return _lifecycle_error(f"topic mismatch expected={live_topic} got={topic}")
    return run_taskctl(
        ["done", task_id, "--runner", CODEX_RUNNER, "--topic", str(live_topic)],
        allow_write=True,
        runner=CODEX_RUNNER,
        topic=str(live_topic),
...
def callback_outbox_status() -> dict[str, Any]:
    ...
    counts, pending/inflight/delivered/dead-letter counts, last delivery).
    Never returns a full origin_thread_id -- taskctl.py's
    ``callback-outbox-status`` command sources this from
    ``AITools/taskdb.py::callback_outbox_stats``, which redacts at the
    source."""
    return run_taskctl(["callback-outbox-status"]).as_dict()
...
def export_jsonl(runner: str | None = None, topic: str | None = None) -> dict[str, Any]:
    ...
    return run_taskctl(["export-jsonl"], allow_write=True, runner=runner, topic=topic).as_dict()
