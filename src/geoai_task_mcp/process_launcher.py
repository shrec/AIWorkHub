"""Local, explicitly gated model-process launcher for GeoAI task workers.
...
try:
    ...
    class _FallbackProjectContext:
        RECEIPT_SCHEMA_ID = "geoai.task_mcp.worker_context_receipt.v1"
...
ALLOW_LAUNCH_ENV = "GEOAI_TASK_MCP_ALLOW_LAUNCH"
ALLOW_WRITES_ENV = "GEOAI_TASK_MCP_ALLOW_WRITES"
MAX_PROCESSES_ENV = "GEOAI_TASK_MCP_MAX_PROCESSES"
EXTERNAL_READONLY_ROOTS: tuple[Path, ...] = (
    Path("/mnt/ssd/geoai_data"),
    Path("/mnt/ssd/corpus"),
    Path("/home/shrek/GeoAI/bitnnv2"),
)
PROCESS_LOG_ENV = "GEOAI_TASK_MCP_PROCESS_LOG_PATH"
PROCESS_DIR_ENV = "GEOAI_TASK_MCP_PROCESS_DIR"
...
HEARTBEAT_LEASE_ENV = "GEOAI_TASK_MCP_HEARTBEAT_LEASE_SECONDS"
QUIET_WARNING_ENV = "GEOAI_TASK_MCP_QUIET_WARNING_SECONDS"
LOST_RECOVERY_GRACE_ENV = "GEOAI_TASK_MCP_LOST_RECOVERY_GRACE_SECONDS"
...
def build_worker_prompt(
    *,
    ...
    return f"""You are the sole worker for one exact GeoAI task in an isolated worktree.
        ...
        *,
        ...
        if not launch_gates_open():
            return self._blocked(
                ...
                "dual_gate_closed: require GEOAI_TASK_MCP_ALLOW_LAUNCH=1 and GEOAI_TASK_MCP_ALLOW_WRITES=1",
            )
        ...
        try:
            ...
            with self._lock, self._registry_lock():
                ...
                metadata = {
                    "schema_id": "geoai.task_mcp.isolated_request.v1",
                    "request_id": request_id,
                    "task_id": task_id,
                    "runner": runner,
                    "topic": topic,
                ...
                thread = threading.Thread(
                    ...
                    args=(live,),
                    name=f"geoai-task-{request_id[:8]}",
                    daemon=True,
                )
                thread.start()
        ...
        *,
        ...
        try:
            ...
            with self._lock, self._registry_lock():
                ...
                child_env["GEOAI_REPO"] = str(self.repo)
        ...
        for request_id, event in candidates.items():
            ...
            if (
                ...
                thread = threading.Thread(
                    ...
                    name=f"geoai-reconcile-{request_id[:8]}",
                    daemon=True,
                )
                thread.start()
                watched += 1
