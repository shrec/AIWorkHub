...
Hard invariants (asserted by tests):
  * READ-ONLY: no code path writes the task queue OR the audit log. The plan
    ...
    not a toggle. This holds even with ``GEOAI_TASK_MCP_ALLOW_WRITES=1``.
  * NO LAUNCH: this module never imports process-spawn machinery and has no
    ...
    env flag (``GEOAI_TASK_MCP_ALLOW_LAUNCH`` / ``_ALLOW_WRITES``) can enable a
...
Neural-bridge note (GeoAI neural-control-first doctrine): the FUTURE selection
...
(``GEOAI_TASK_MCP_COLLISION_CARDS_PATH``, falling back to the existing
...
from geoai_task_mcp import cli_adapter_dryrun as dryrun
...
_COLLISION_CARDS_ENV = "GEOAI_TASK_MCP_COLLISION_CARDS_PATH"
...
def collision_cards_path(repo: Any = None) -> Path:
    ...
    Priority: ``GEOAI_TASK_MCP_COLLISION_CARDS_PATH`` (dedicated, test-safe
...
READONLY_TOOLS: dict[str, Any] = {
    "geoai_cli_adapter_plan_readonly": plan_command_readonly,
    "geoai_cli_adapter_audit_summary_readonly": audit_summary_readonly,
    "geoai_cli_adapter_report_readonly": readonly_tool_report,
}
