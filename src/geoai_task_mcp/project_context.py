...
SCHEMA_ID = "geoai.task_mcp.project_context_bundle.v1"
...
def _json_hit_count(value: Any) -> int:
    ...
    if isinstance(value, dict):
        if value.get("schema_id") == "geoai.task_mcp.bounded_json_preview.v1":
...
def _source_graph_argv(repo: Path, contract: dict[str, Any]) -> list[str]:
    script = repo / "AITools" / "source_graph.py"
...
def collect_project_context(repo: Path, card: dict[str, Any]) -> ProjectContextResult | None:
    ...
    for relative in ("AITools/source_graph.py", "AITools/transcript_graph.py"):
    ...
    try:
        source_text, source_truncated = _run_fixed_argv(_source_graph_argv(repo, contract), cwd=repo)
    ...
    metadata = {
        ...
        "optimization": {
            "schema_id": "geoai.task_mcp.project_context_optimization.v1",
