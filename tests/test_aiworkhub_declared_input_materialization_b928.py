...
def _ensure_aiworkhub_sibling_stubs() -> None:
    ...
    for sub in _ABSENT_SIBLINGS:
        ...
        if full_name in sys.modules:
            continue
        try:
            # In a full checkout the real sibling imports cleanly; only stub a
            # genuinely absent module (sparse worktree). Never let a stub shadow
            # a real module in ``sys.modules`` -- that leaks into unrelated
            # modules' collection-time checks for the rest of the suite (e.g. a
            # stubbed ``worker_workspace.landlock_abi_version()`` returning None
            # crashing another suite's ``... < 1`` skipif at import).
