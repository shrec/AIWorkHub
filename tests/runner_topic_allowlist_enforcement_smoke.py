...
``geoai_task_mcp.core.run_taskctl`` / ``check_runner_topic_allowlist``:
  ...
  3. Layer ORDER is preserved: the existing GEOAI_TASK_MCP_ALLOW_WRITES gate is
...
temp audit log path (GEOAI_TASK_MCP_AUDIT_LOG_PATH). The REAL parent queue and
...
REAL_AUDIT_LOG = REPO / "tools" / "geoai-task-mcp" / "logs" / "audit.jsonl"
...
def test_write_gate_checked_before_allowlist() -> None:
    ...
    saved = {k: os.environ.get(k) for k in ("GEOAI_TASK_MCP_ALLOW_WRITES", "GEOAI_TASK_MCP_AUDIT_LOG_PATH")}
    tmp = Path(tempfile.mkdtemp(prefix="geoai_b119_gateorder_"))
    try:
        os.environ["GEOAI_TASK_MCP_ALLOW_WRITES"] = "0"
        os.environ["GEOAI_TASK_MCP_AUDIT_LOG_PATH"] = str(tmp / "audit.jsonl")
...
def test_allowlist_denies_without_subprocess() -> None:
    ...
    saved = {k: os.environ.get(k) for k in ("GEOAI_TASK_MCP_ALLOW_WRITES", "GEOAI_TASK_MCP_AUDIT_LOG_PATH",
                                             "GEOAI_TASK_MCP_SECRET_PROBE")}
    tmp = Path(tempfile.mkdtemp(prefix="geoai_b119_denyaudit_"))
    try:
        os.environ["GEOAI_TASK_MCP_ALLOW_WRITES"] = "1"
        os.environ["GEOAI_TASK_MCP_AUDIT_LOG_PATH"] = str(tmp / "audit.jsonl")
        os.environ["GEOAI_TASK_MCP_SECRET_PROBE"] = "super-secret-value-must-never-appear-in-audit"
...
def test_malformed_audit_tag() -> None:
    ...
    saved = {k: os.environ.get(k) for k in ("GEOAI_TASK_MCP_ALLOW_WRITES", "GEOAI_TASK_MCP_AUDIT_LOG_PATH")}
    tmp = Path(tempfile.mkdtemp(prefix="geoai_b119_malformedtag_"))
    try:
        os.environ["GEOAI_TASK_MCP_ALLOW_WRITES"] = "1"
        os.environ["GEOAI_TASK_MCP_AUDIT_LOG_PATH"] = str(tmp / "audit.jsonl")
...
def test_allowed_identity_proceeds_isolated() -> None:
    saved = {k: os.environ.get(k) for k in (
        ...
        "GEOAI_REPO", "GEOAI_TASK_MCP_ALLOW_WRITES", "GEOAI_TASK_MCP_AUDIT_LOG_PATH",
    )}
    tmp = Path(tempfile.mkdtemp(prefix="geoai_b119_allow_isolated_"))
    try:
...
def test_card_scoped_claim_review_usage_authority() -> None:
    saved_env = {k: os.environ.get(k) for k in (
        "GEOAI_TASK_MCP_ALLOW_WRITES", "GEOAI_TASK_MCP_AUDIT_LOG_PATH",
    )}
    saved_show = core.show_task
    saved_run = core.subprocess.run
    tmp = Path(tempfile.mkdtemp(prefix="geoai_b623_card_scoped_"))
    calls: list[list[str]] = []
