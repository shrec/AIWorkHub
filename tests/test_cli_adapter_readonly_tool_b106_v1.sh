...
#   3. the plan tool appends NO audit entry even with AIWORKHUB_ALLOW_WRITES=1
...
TMPDIR_AUDIT="$(mktemp -d "${TMPDIR:-/tmp}/aiworkhub_cli_adapter_readonly_sh.XXXXXX")"
...
RO_LINES="$(AIWORKHUB_ALLOW_WRITES=1 AIWORKHUB_AUDIT_LOG_PATH="$RO_AUDIT" python3 -c '
import os, sys
sys.path.insert(0, "'"$MCPROOT"'/src")
from aiworkhub import cli_adapter_readonly_tool as m
