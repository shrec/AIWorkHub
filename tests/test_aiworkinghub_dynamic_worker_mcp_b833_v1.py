...
from geoai_task_mcp import agent_tool_instructions as instr  # noqa: E402
...
def _write_storage_registry(repo: Path) -> None:
    """Minimal fake ``.aiworkinghub/config/storage.json`` -- every component
...
def _ctx(
    ...
    runtime = w.generate_worker_mcp_runtime(
        ...
        source_graph_targets=list(targets), session_topic="AIWorkingHub dynamic worker MCP B833",
    )
    return w.WorkerToolContext(
        ...
        session_topic="AIWorkingHub dynamic worker MCP B833",
...
def test_fake_worker_end_to_end_dynamic_tool_call(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    dynamic_call = fake_mcp.registered["geoai_worker_source_graph_query"]
...
def test_eval_artifact_b833_matches_live_tool_surface() -> None:
    path = (
        ...
        / "eval"
        / "aiworkinghub_dynamic_worker_mcp_b833_v1.json"
    )
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert payload["schema_id"] == "geoai.task_mcp.aiworkinghub_dynamic_worker_mcp_b833_v1.eval.v1"
