...
# ---------------------------------------------------------------------------
# main(): CLI entrypoint fails closed with no manual global fallback
# ---------------------------------------------------------------------------

def test_main_fails_closed_without_repo_id_env(monkeypatch, capsys):
    monkeypatch.delenv(asm.ENV_REPO_ID, raising=False)
    rc = asm.main(["app-server", "--listen", "stdio://"])
    assert rc == 1
    assert asm.ENV_REPO_ID in capsys.readouterr().err
