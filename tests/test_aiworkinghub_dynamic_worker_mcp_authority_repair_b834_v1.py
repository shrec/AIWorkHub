...
dual binding. Companion to test_aiworkinghub_dynamic_worker_mcp_b833_v1.py,
...
from geoai_task_mcp import runtime_adapters  # noqa: E402
from geoai_task_mcp import worker_ai_tools_mcp as w  # noqa: E402
...
def _write_storage_registry(repo: Path, *, canonical_active: bool = False) -> None:
    ...
    path = repo / ".aiworkinghub" / "config" / "storage.json"
    path.parent.mkdir(parents=True, exist_ok=True)
...
def _authority_repo(tmp_path: Path, *, name: str = "authority_repo", echo_argv: bool = False) -> Path:
    ...
    source_graph_body = (
        ...
        " 'relevant_files': ['AITools/source_graph.py', 'tools/geoai-task-mcp/src/geoai_task_mcp/x.py']}))\n"
    )
...
def _ctx(
    *, repo: Path, authority_repo: Path, home: Path,
    ...
    runtime = w.generate_worker_mcp_runtime(
        ...
        source_graph_targets=list(targets), session_topic="AIWorkingHub dynamic worker MCP B834 authority repair",
    )
    return w.WorkerToolContext(
        ...
        session_topic="AIWorkingHub dynamic worker MCP B834 authority repair",
...
def test_eval_artifact_b834_matches_live_defect_repair() -> None:
    path = (
        ...
        / "eval"
        / "aiworkinghub_dynamic_worker_mcp_authority_repair_b834_v1.json"
    )
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert payload["schema_id"] == "geoai.task_mcp.aiworkinghub_dynamic_worker_mcp_authority_repair_b834_v1.eval.v1"
    ...
    b829_paths = {
        "tools/geoai-task-mcp/eval/aiworkinghub_context_optimization_b829_v1.json",
        "tools/geoai-task-mcp/eval/aiworkinghub_context_optimization_rows_b829_v1.jsonl",
    }
