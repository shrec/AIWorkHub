...
from geoai_task_mcp import task_store_migration as migrator
from geoai_task_mcp.repository_state import bootstrap_repository
...
def test_shadow_uses_sqlite_backup_api_with_active_wal_and_preserves_parity(tmp_path: Path) -> None:
    ...
    assert result["paths"]["canonical_db"].endswith(".aiworkinghub/tasking/task_queue.sqlite")
...
def test_evidence_json_is_valid() -> None:
    payload = json.loads(
        (REPO / "tools/geoai-task-mcp/eval/aiworkinghub_taskdb_migrator_b825_v1.json").read_text()
    )
    assert payload["task_id"] == "CODEX_GPT55_AIWORKINGHUB_TASKDB_MIGRATOR_B825_V1"
