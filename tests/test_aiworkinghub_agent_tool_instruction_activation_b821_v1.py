...
from geoai_task_mcp import agent_tool_instruction_cli as cli
...
@mock.patch("geoai_task_mcp.agent_tool_instruction_mcp.core.writes_allowed", return_value=True)
...
@mock.patch("geoai_task_mcp.agent_tool_instruction_mcp.core.writes_allowed", return_value=False)
...
def test_eval_artifact_activation_b821_v1():
    ...
    path = Path(
        "tools/geoai-task-mcp/eval/aiworkinghub_agent_tool_instruction_activation_b821_v1.json"
    )
    payload = json.loads(path.read_text(encoding="utf-8"))
    assert (
        payload["schema_id"]
        == "geoai.task_mcp.agent_tool_instruction_activation_b821_v1.eval.v1"
    )
