...
# the tripwire only ever appends a notice, and the guard refuses a
...
def test_zero_delta_tripwire_never_cancels_a_live_worker(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    manager = _manager(
        tmp_path,
    ...
    assert notice["enforced"] is False
