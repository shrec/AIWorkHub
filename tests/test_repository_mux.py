...
from aiworkhub.repository_mux import RepositoryDescriptor, RepositoryWindowMux
from aiworkhub.repository_state import bootstrap_repository, inspect_repository
from aiworkhub.route_identity import CoordinatorRouteKey, fail_on_incomplete_coordinator_route
...
def test_crash_restart_rehydrates_selected_repo_without_cross_reads(tmp_path: Path) -> None:
    ...
    assert (session.repository.root / ".aiworkhub" / "tasking" / "TASK.SAME.json").read_text(
        encoding="utf-8"
    ).strip() == '{"repo": "beta"}'
...
def test_coordinator_routes_require_repo_provider_window_and_thread_or_session(tmp_path: Path) -> None:
    repo = _repo(tmp_path, "alpha", "repo_alpha000000000000000000000000001")
    mux = RepositoryWindowMux()
    mux.open_repository("window_a", repo, claim_episode="episode_origin_00000000001")

    codex = mux.persist_coordinator_callback(
        "window_a",
        provider="codex",
        thread_id="thread.same",
        task_id="TASK.SAME",
        callback_event_id="event.done",
        payload={"provider": "codex"},
    )
    claude = mux.persist_coordinator_callback(
        "window_a",
        provider="claude",
    ...
    assert codex.canonical() != claude.canonical()
    assert mux.deliver_coordinator_callback(
    ...
    with pytest.raises(ValueError, match="provider is required"):
        fail_on_incomplete_coordinator_route(
            repo_id=repo.manifest.repo_id,
            window_id="window_a",
            thread_id="thread.same",
            task_id="TASK.SAME",
    ...
    with pytest.raises(ValueError, match="codex callback route requires thread_id"):
        CoordinatorRouteKey(
            repo_id=repo.manifest.repo_id,
            provider="codex",
            window_id="window_a",
...
def test_parallel_repositories_have_disjoint_route_keys_and_state_fingerprints(tmp_path: Path) -> None:
    repo_a = _repo(tmp_path, "alpha", "repo_alpha000000000000000000000000001")
    repo_b = _repo(tmp_path, "beta", "repo_beta0000000000000000000000000002")
    mux = RepositoryWindowMux()
    mux.open_repository("window_a", repo_a, claim_episode="episode_same_claim_0001")
    mux.open_repository("window_b", repo_b, claim_episode="episode_same_claim_0001")

    route_a = mux.persist_coordinator_callback(
        "window_a",
        provider="codex",
        thread_id="thread.same",
        task_id="TASK.SAME",
    ...
    route_b = mux.persist_coordinator_callback(
        "window_b",
        provider="codex",
        thread_id="thread.same",
        task_id="TASK.SAME",
    ...
    assert repo_a.hub_dir != repo_b.hub_dir
    assert repo_a.durable_paths["tasking"] != repo_b.durable_paths["tasking"]
    assert (repo_a.hub_dir / "config" / "storage.json") != (repo_b.hub_dir / "config" / "storage.json")
    assert mux.deliver_callback(route_a).payload == {"repo": "alpha"}
    assert mux.deliver_callback(route_b).payload == {"repo": "beta"}

    wrong_repo_route = CoordinatorRouteKey(
        repo_id=repo_b.manifest.repo_id,
        provider="codex",
        window_id="window_a",
        thread_id="thread.same",
        task_id="TASK.SAME",
    ...
    assert mux.deliver_callback(wrong_repo_route) is None
