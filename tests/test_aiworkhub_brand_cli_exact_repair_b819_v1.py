from __future__ import annotations

import importlib
import re
import sys
import types
from pathlib import Path

import pytest


ROOT = Path(__file__).resolve().parents[1]
SRC = ROOT / "src"
PYPROJECT = ROOT / "pyproject.toml"

if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))


class _FastMCPStub:
    def __init__(self, name: str):
        self.name = name
        self.registered_tools: list[str] = []
        self.run_called = False

    def tool(self, *args, **kwargs):
        def decorate(func):
            self.registered_tools.append(kwargs.get("name") or func.__name__)
            return func

        return decorate

    def run(self):
        self.run_called = True


def _install_server_import_stubs(monkeypatch):
    fastmcp_mod = types.ModuleType("mcp.server.fastmcp")
    fastmcp_mod.FastMCP = _FastMCPStub
    server_pkg = types.ModuleType("mcp.server")
    server_pkg.fastmcp = fastmcp_mod
    mcp_pkg = types.ModuleType("mcp")
    mcp_pkg.server = server_pkg
    monkeypatch.setitem(sys.modules, "mcp", mcp_pkg)
    monkeypatch.setitem(sys.modules, "mcp.server", server_pkg)
    monkeypatch.setitem(sys.modules, "mcp.server.fastmcp", fastmcp_mod)

    dashboard_mod = types.ModuleType("aiworkhub.dashboard_mcp_app")

    def register(mcp):
        @mcp.tool()
        def aiworkhub_dashboard_snapshot():
            return {"ok": True}

        @mcp.tool()
        def aiworkhub_dashboard_openapi():
            return {"ok": True}

        @mcp.tool()
        def aiworkhub_dashboard_static_manifest():
            return {"ok": True}

    dashboard_mod.register = register
    monkeypatch.setitem(sys.modules, "aiworkhub.dashboard_mcp_app", dashboard_mod)
    package = sys.modules.get("aiworkhub")
    if package is not None:
        monkeypatch.setattr(package, "dashboard_mcp_app", dashboard_mod, raising=False)

    deepseek_mod = types.ModuleType("aiworkhub.deepseek_credentials")
    deepseek_mod.adapter_readiness = lambda repo=None: {"ok": True, "repo": str(repo) if repo else None}
    deepseek_mod.main = lambda: None
    monkeypatch.setitem(sys.modules, "aiworkhub.deepseek_credentials", deepseek_mod)
    if package is not None:
        monkeypatch.setattr(package, "deepseek_credentials", deepseek_mod, raising=False)


def _drop_imports():
    for name in [
        "aiworkhub.server",
        "aiworkhub",
        "aiworkhub.server",
    ]:
        sys.modules.pop(name, None)
    legacy_package = sys.modules.get("aiworkhub")
    if legacy_package is not None:
        legacy_package.__dict__.pop("server", None)
    branded_package = sys.modules.get("aiworkhub")
    if branded_package is not None:
        branded_package.__dict__.pop("server", None)


@pytest.fixture(autouse=True)
def _isolate_server_alias_imports():
    yield
    _drop_imports()


def test_aiworkhub_server_alias_is_same_fastmcp_server(monkeypatch):
    _drop_imports()
    _install_server_import_stubs(monkeypatch)

    legacy = importlib.import_module("aiworkhub.server")
    branded = importlib.import_module("aiworkhub.server")

    assert branded.mcp is legacy.mcp
    assert branded.main is legacy.main
    assert branded.aiworkhub_task_health is legacy.aiworkhub_task_health
    assert branded.dashboard_mcp_app is legacy.dashboard_mcp_app
    assert branded.core is legacy.core


def test_dashboard_mcp_app_registration_survives_legacy_import(monkeypatch):
    _drop_imports()
    _install_server_import_stubs(monkeypatch)

    legacy = importlib.import_module("aiworkhub.server")

    assert hasattr(legacy, "dashboard_mcp_app")
    assert legacy.dashboard_mcp_app is sys.modules["aiworkhub.dashboard_mcp_app"]
    assert "aiworkhub_dashboard_snapshot" in legacy.mcp.registered_tools
    assert "aiworkhub_dashboard_openapi" in legacy.mcp.registered_tools
    assert "aiworkhub_dashboard_static_manifest" in legacy.mcp.registered_tools


def test_legacy_and_branded_console_entry_points_resolve_to_same_main():
    scripts: dict[str, str] = {}
    in_scripts = False
    for line in PYPROJECT.read_text(encoding="utf-8").splitlines():
        stripped = line.strip()
        if stripped.startswith("["):
            in_scripts = stripped == "[project.scripts]"
            continue
        if not in_scripts:
            continue
        match = re.fullmatch(r'([A-Za-z0-9_-]+)\s*=\s*"([^"]+)"', stripped)
        if match:
            scripts[match.group(1)] = match.group(2)

    assert scripts["aiworkhub"] == "aiworkhub.server:main"
    assert scripts["aiworkhub"] == scripts["aiworkhub"]
    assert scripts["awh"] == scripts["aiworkhub"]
    assert scripts["aiworkhub-mcp"] == scripts["aiworkhub"]


def test_aiworkhub_package_exports_canonical_version():
    import aiworkhub
    import aiworkhub

    assert aiworkhub.__version__ == aiworkhub.__version__
