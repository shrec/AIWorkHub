...
from geoai_task_mcp import fresh_task_store
...
def _storage(repo_id: str) -> dict:
    return {
        "schema_id": "aiworkinghub.storage_registry.v1",
        "registry_version": 1,
        "repo_id": repo_id,
        "portable": True,
...
def _make_repo(tmp_path: Path, repo_id: str = "repo_test_b832") -> Path:
    repo = tmp_path / "repo"
    (repo / ".aiworkinghub" / "config").mkdir(parents=True)
    (repo / "bitnnv2" / "data" / "tasking").mkdir(parents=True)
    (repo / "AITools").mkdir()
    shutil.copyfile(REPO / "AITools" / "taskdb.py", repo / "AITools" / "taskdb.py")
    (repo / ".aiworkinghub" / "project.json").write_text(
        json.dumps({
            "schema_id": "aiworkinghub.project_manifest.v1",
            "repo_id": repo_id,
            "repo_name": "Temp",
    ...
    (repo / ".aiworkinghub" / "config" / "storage.json").write_text(
...
def test_fresh_cutover_archives_legacy_and_publishes_empty_canonical_db(tmp_path: Path) -> None:
    ...
    before_registry = json.loads((repo / ".aiworkinghub" / "config" / "storage.json").read_text())

    dry = fresh_task_store.cutover(repo, execute=False)
    result = fresh_task_store.cutover(repo, execute=True)

    archive = repo / ".aiworkinghub" / fresh_task_store.ARCHIVE_REL
    canonical = repo / ".aiworkinghub" / fresh_task_store.CANONICAL_REL
    after_registry = json.loads((repo / ".aiworkinghub" / "config" / "storage.json").read_text())
...
def test_rollback_restores_legacy_authority_without_overwriting_archive(tmp_path: Path) -> None:
    ...
    registry = json.loads((repo / ".aiworkinghub" / "config" / "storage.json").read_text())
...
def test_taskctl_execute_requires_coordinator_token(tmp_path: Path) -> None:
    ...
    env = {
        **os.environ,
        ...
        "PYTHONPATH": str(REPO / "tools" / "geoai-task-mcp" / "src"),
    }
    ok = subprocess.run(
        [
            sys.executable,
...
def test_taskdb_default_binds_to_active_registry_authority(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert active_module.DEFAULT_DB == repo / ".aiworkinghub" / "tasking" / "task_queue.sqlite"
