...
from geoai_task_mcp import glm_credentials as gc  # noqa: E402
from geoai_task_mcp import process_launcher, runtime_adapters  # noqa: E402
...
def test_glm_credential_default_path_is_under_aiworkinghub(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    home = tmp_path / "home"
    monkeypatch.setenv("HOME", str(home))
    monkeypatch.delenv(gc.CREDENTIAL_PATH_ENV, raising=False)
    assert gc.credential_path() == home / ".config" / "aiworkinghub" / "glm_copilot_credential.json"
...
def test_glm_status_is_secret_free_and_names_setup_status_commands(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert status["setup_command"] == "python3 -m geoai_task_mcp.glm_credentials setup"
    assert status["status_command"] == "python3 -m geoai_task_mcp.glm_credentials status"
