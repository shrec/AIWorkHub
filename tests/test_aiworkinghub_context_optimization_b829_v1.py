...
from geoai_task_mcp.context_cache import (
    ContextQueryKey,
    query_cache_key_sha256,
    query_key_material,
)
from geoai_task_mcp.context_economics import context_optimization_report
from geoai_task_mcp.project_context import (
    ProjectContextError,
    TOOL_CAPS,
...
TASK_ID = "CODEX_GPT55_AIWORKINGHUB_CONTEXT_OPTIMIZATION_B829_V1"
RUNNER = "codex_gpt55_aiworkinghub_context_optimization_b829_v1"
EVAL_JSON = Path("tools/geoai-task-mcp/eval/aiworkinghub_context_optimization_b829_v1.json")
EVAL_ROWS = Path("tools/geoai-task-mcp/eval/aiworkinghub_context_optimization_rows_b829_v1.jsonl")
PROCESS_DIRS = [
    Path("/home/shrek/GeoAI/tools/geoai-task-mcp/logs/processes"),
...
def _base_card() -> dict:
    return {
        ...
        "allowed_writes": ["tools/geoai-task-mcp/src/geoai_task_mcp/project_context.py"],
        "project_context": {
            "required": True,
            "task_type": "code",
            "source_graph": {
                ...
                "budget": 64,
                "required": True,
                "targets": ["tools/geoai-task-mcp/src/geoai_task_mcp/project_context.py"],
            },
            "session": {"topic": "AIWorkingHub context optimization", "limit": 5},
...
def test_project_context_caps_zero_hit_suppression_and_fail_closed(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("geoai_task_mcp.project_context._run_fixed_argv", fake_run)
    ...
    assert sections["ai_memory"]["content"] == ""
    assert sections["kb"]["content"] == ""
    assert sections["ai_memory"]["suppression_reason"] == "zero_hit_optional_tool"
    ...
    monkeypatch.setattr("geoai_task_mcp.project_context._run_fixed_argv", empty_source_graph)
...
def test_real_process_context_optimization_evidence_written():
    ...
    EVAL_ROWS.write_text(
        ...
        or json.dumps({
            "schema_id": "geoai.task_mcp.context_optimization.row.b829.v1",
