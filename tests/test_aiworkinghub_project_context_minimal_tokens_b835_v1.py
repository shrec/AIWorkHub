...
from geoai_task_mcp import project_context  # noqa: E402

TASK_ID = "CLAUDE_SONNET5_AIWORKINGHUB_PROJECT_CONTEXT_MINIMAL_TOKENS_B835_V1"
RUNNER = "claude_sonnet5_aiworkinghub_project_context_minimal_tokens_b835_v1"
EVAL_JSON = (
    Path(__file__).resolve().parents[1]
    / "eval"
    / "aiworkinghub_project_context_minimal_tokens_b835_v1.json"
)
...
SOURCE_GRAPH_PAYLOAD = {
    ...
    "relevant_files": [
        "tools/geoai-task-mcp/src/geoai_task_mcp/project_context.py",
    ],
    "risks": [],
...
SESSION_PAYLOAD = {
    "topic": "AIWorkingHub project context minimal tokens B835",
    "state": "current",
    "evidence_count": 5,
    "current_source": {
        "kind": "commit",
...
def test_envelope_overhead_stays_bounded_as_evidence_scales(tmp_path: Path) -> None:
    ...
    large_source["relevant_files"] = [
        f"tools/geoai-task-mcp/src/geoai_task_mcp/module_{i}.py" for i in range(60)
    ]
...
def test_real_measurement_evidence_written(tmp_path: Path) -> None:
    ...
    report = {
        "schema_id": "geoai.task_mcp.project_context_minimal_tokens.b835.v1",
        "task_id": TASK_ID,
        "runner": RUNNER,
        "verdict": "PASS",
        "fixture": {
