...
# (aiworkhub_launch_queue_describe_readonly / _evaluate_readonly /
...
#      decision=blocked_launch_disabled -- with AIWORKHUB_ALLOW_LAUNCH
...
TMPDIR_STATE="$(mktemp -d "${TMPDIR:-/tmp}/aiworkhub_launch_queue_ro_wiring_sh.XXXXXX")"
trap 'rm -rf "$TMPDIR_STATE"' EXIT

export PYTHONPATH="$MCPROOT/src"
export AIWORKHUB_REPO="$ROOT"
export AIWORKHUB_AUDIT_LOG_PATH="$TMPDIR_STATE/task_audit.jsonl"
export AIWORKHUB_LAUNCH_QUEUE_LOG_PATH="$TMPDIR_STATE/launch_queue_audit.jsonl"
...
from aiworkhub import launch_queue_contract as lqc
from aiworkhub import launch_queue_persist as lqp
from aiworkhub import cli_adapter_readonly_tool as ro

STATE_DIR = Path(os.environ["AIWORKHUB_AUDIT_LOG_PATH"]).parent
LAUNCH_LOG = Path(os.environ["AIWORKHUB_LAUNCH_QUEUE_LOG_PATH"])
...
def _check_gate_invariants(allow_launch):
    if allow_launch is None:
        ...
        os.environ["AIWORKHUB_ALLOW_LAUNCH"] = allow_launch
        label = allow_launch

    _content, described = _call("aiworkhub_launch_queue_describe_readonly", describe_args)
    ...
    _content, evaluated = _call("aiworkhub_launch_queue_evaluate_readonly", evaluate_args)
    ...
    print(f"PASS: gate invariants hold with AIWORKHUB_ALLOW_LAUNCH={label}")
...
def _run_round(allow_writes, allow_launch):
    for name, val in (
        ("AIWORKHUB_ALLOW_WRITES", allow_writes),
        ("AIWORKHUB_ALLOW_LAUNCH", allow_launch),
    ):
        if val is None:
    ...
    for name, args in (
        ("aiworkhub_launch_queue_describe_readonly", describe_args),
        ("aiworkhub_launch_queue_evaluate_readonly", evaluate_args),
        ("aiworkhub_launch_queue_audit_summary_readonly", {"max_entries": 10}),
    ):
...
_content, summary = _call("aiworkhub_launch_queue_audit_summary_readonly", {"max_entries": 10})
