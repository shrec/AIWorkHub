...
from geoai_task_mcp import repository_state as rs
...
def test_two_same_named_repositories_keep_distinct_ids(tmp_path: Path) -> None:
    first = tmp_path / "one" / "GeoAI"
    second = tmp_path / "two" / "GeoAI"
    _git_init(first)
    _git_init(second)
    ...
    assert a.root.name == b.root.name == "GeoAI"
...
def test_resolver_precedence_explicit_env_manifest_then_git(tmp_path: Path) -> None:
    ...
    env = {"AIWORKINGHUB_REPO_ROOT": str(env_repo)}
